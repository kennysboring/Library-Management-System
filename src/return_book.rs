...
pub fn return_book(list: &mut Vec<Book>) -> Result<(), ERRO>{
    let mut id: String = String::with_capacity(2);

    println!("Write the id book you want to return");
    io::stdin().read_line(&mut id).map_err(|_|ERRO::ErrorReadReturnBooK)?;

    let id_as_number: usize = id.trim().parse().map_err(|_|ERRO::ErrorIDReturnBook)?;
