...
impl Library {
    ...
    pub fn add_book(&mut self, title: String, author: String) {
        ...
        let new_book = Book {
            //variavel com as informações dos livros
            name: title,
            author: author,
            borrowable: true,
            id: self.next_id,
    ...
    fn verify_borrowable(&self, index: usize) -> Status {
        match self.book.get(index) {
            Some(b) if b.borrowable => Status::Available,
    ...
    fn book_to_line(book: &Book) -> String {
        //formata as informações do livro em uma linha e retorna ela
        format!(
            "{};{};{};{}\n",
            book.id,
            book.name,
    ...
    pub fn save_in_file(&self) {
        ...
        let mut file = File::create(FILE_PATH).expect("ERROR to create/open a file books.txt");

        for book in &self.book {
            //passa por cada livro e adiciona em 'books.txt.'
    ...
    fn load_from_file(&mut self) {
        if !Path::new(FILE_PATH).exists() {
            //sai da função se o arquivo não existe
            return;
        }
        ...
        for line in reader.lines() {
            ...
            if parts.len() != 4 {
                //lida com erros caso tenha mais de 4 partes
                println!("Line invaliable: {}", line);
                continue; //pula para o proximo valor de 'line'
            }

            let id: u32 = match parts[0].trim().parse() {
                //transforma o primeiro valor em um número e trata o erro
                Ok(v) => v,
                Err(_) => {
            ...
            let book = Book {
                //salva as informações da linha em uma variavel
                id,
            ...
            if id >= self.next_id {
                //faz a verificão do id para o quando for adicionar o proximo livro
                self.next_id = id;
            }
